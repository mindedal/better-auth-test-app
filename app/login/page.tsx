      ...
      await authClient.signIn.email({
        ...
        onSuccess: async (ctx) => {
             ...
             if (data?.user || data?.session) {
                console.log("User/Session data found. Redirecting...");
             ...
             console.log("Response data missing user/session. Verifying session...");
             try {
                const sessionData = await authClient.getSession();
                
                if (sessionData.data) {
                    console.log("Session verified successfully via fetch.");
                    toast.success("Signed in successfully!")
                    window.location.href = "/dashboard"
                } else {
                    console.error("SignIn successful but no session established.", data);
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-100">
          <Card className="w-[350px]">
            ...
            <CardFooter className="flex flex-col gap-2">
              <Button className="w-full" onClick={handle2FAVerify} disabled={isPending || otpCode.length < 6}>
                  {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Verify
              </Button>
              <Button variant="ghost" className="w-full" onClick={() => setIsTwoFactor(false)}>
                Back to Login
              </Button>
            </CardFooter>
          </Card>
